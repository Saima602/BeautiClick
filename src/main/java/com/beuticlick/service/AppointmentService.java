...
public class AppointmentService {
	...
	public Appointment book(Appointment appointment, Long salonId) {
		...
		// Snapshot the price at booking time — billing stays accurate even if price
		// changes later
		SalonService svc = salonServiceService.getById(appointment.getService().getId(), salonId);
	...
	public Appointment getById(Long id, Long salonId) {
		Appointment appt = repo.findById(id).orElseThrow(() -> new RuntimeException("Appointment not found: " + id));
	...
	public void completeAppointment(Long appointmentId, Long salonId) {
		...
		bill.setCustomerId(appt.getCustomer().getId());
